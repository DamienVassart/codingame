...
very second, depending on the current flight parameters (location, speed, fuel ...), 
...
The game simulates a free fall without atmosphere. Gravity on Mars is 3.711 m/s² . For a thrust power of X, a push force equivalent to X m/s² is generated and X liters of fuel are consumed. 
...
while (true) {
    var inputs = readline().split(' ');
    const X = parseInt(inputs[0]); // X coodinates
    const Y = parseInt(inputs[1]); // Y coordinates
    ...
    var angle = rotate, thrust = power;

    thrust = (vSpeed <= -40) ? 4 : 0;
